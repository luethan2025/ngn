...
def get_argparser():
  ...
  parser.add_argument("--circle_one_radius", type=int, default=1,
                      help="circle one radius (default: 1)")
  parser.add_argument("--circle_two_radius", type=int, default=3,
                      help="circle two radius (default: 1)")
...
def get_dataset(num_points, split_ratio, \
                circle_one_radius, circle_two_radius, \
                batch_size, visualize=False):
  num_class_one = int(num_points * split_ratio)
  angles_one = np.random.uniform(0, 2 * np.pi, num_class_one)
  radii_one = np.sqrt(np.random.uniform(0, circle_one_radius, num_class_one))
  x_one = radii_one * np.cos(angles_one)
  y_one = radii_one * np.sin(angles_one)
  ...
  radii_two = np.sqrt(np.random.uniform(circle_two_radius, circle_one_radius, num_class_two))
  x_two = radii_two * np.cos(angles_two)
  y_two = radii_two * np.sin(angles_two)
...
def main():
  ...
  train_dataset = get_dataset(opts.num_points, opts.split_ratio, \
                              opts.circle_one_radius, opts.circle_two_radius, \
