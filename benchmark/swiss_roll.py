...
def get_argparser():
  ...
  parser.add_argument("--spread_factor", type=float, default=0.1,
                      help="spread factor (default: 0.1)")
  ...
  # Visualizer Options
  parser.add_argument("--visualize_dataset", action="store_true", default=True,
                      help="visualize dataset (default: False)")
  parser.add_argument("--visualize_loss_and_acc", action="store_true", default=False,
                      help="visualize dataset (default: False)")
  return parser

def get_dataset(num_points, split_ratio, noise, batch_size, visualize=False):
  num_class_one = int(num_points * split_ratio)
  theta_one = np.linspace(0, 4 * np.pi, num_class_one)
  z_one = np.linspace(-10, 10, num_class_one)
  x_noise = np.random.normal(0, noise, num_class_one)
  y_noise = np.random.normal(0, noise, num_class_one)
  x_one = (1 + 0.1 * z_one) * np.sin(theta_one) + x_noise
  y_one = (1 + 0.1 * z_one) * np.cos(theta_one) + y_noise
  class_one = np.column_stack((x_one, y_one))

  num_class_two = int(num_points - (num_points * split_ratio))
  theta_two = np.linspace(0, 4 * np.pi, num_class_two)
  z_two = np.linspace(-10, 10, num_class_two)
  x_noise = np.random.normal(0, noise, num_class_one)
  y_noise = np.random.normal(0, noise, num_class_one)
  x_two = -(1 + 0.1 * z_two) * np.sin(theta_two) + x_noise
  y_two = -(1 + 0.1 * z_two) * np.cos(theta_two) + y_noise
...
def main():
  ...
  train_dataset = get_dataset(opts.num_points, opts.split_ratio, opts.spread_factor, opts.batch_size, visualize=opts.visualize_dataset)
  ...
  for i in range(opts.num_test_dataset):
    ...
    test_dataset = get_dataset(opts.num_points, opts.split_ratio, opts.spread_factor, opts.batch_size)
